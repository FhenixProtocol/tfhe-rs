...
impl ClientKey {
    ...
    /// assert_eq!(dec, true);
    ///
    /// // Here the value is not a boolean
    /// // This is ok for decrypt function as it treats non zero as true
    /// // However, generally when a function requires an input to be wrapped in
    /// // BooleanBlock, it means the ciphertext must be 0 or 1
    /// let a = cks.encrypt_one_block(15u64);
    /// let wrapped = BooleanBlock::new_unchecked(a);
    /// let dec = cks.decrypt_bool(&wrapped);
    /// assert_eq!(dec, true);
    /// ```
